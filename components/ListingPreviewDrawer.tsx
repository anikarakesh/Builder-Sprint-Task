...
export default function ListingPreviewDrawer({ open, onClose, selected }: Props) {
    ...
    <>
      ...
      <aside className={
        ...
        <div className="p-5 border-b border-[#e1e4ea] flex items-center justify-between">
          <div className="text-[18px] font-medium">{selected?.name || "Project Name"}</div>
        ...
        <div className="px-5 pt-3 pb-4 border-b border-[#e1e4ea]">
          <div className="flex items-center gap-4 text-[14px]">
            <button onClick={() => setTab("Details")} className={"pb-2 " + (tab === "Details" ? "border-b-2 border-[#0062e0] text-[#0e121b] font-medium" : "text-[#525866]")}>Details</button>
            <button onClick={() => setTab("Analytics")} className={"pb-2 " + (tab === "Analytics" ? "border-b-2 border-[#0062e0] text-[#0e121b] font-medium" : "text-[#525866]")}>Analytics</button>
          </div>
        </div>
          ...
          {tab === "Analytics" && (
            <div className="text-[14px] text-[#0e121b]">
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg border border-[#e1e4ea] p-3">
                  <div className="text-[12px] text-[#99a0ae] uppercase">Platform</div>
                  <div className="mt-1 font-medium">{selected?.platform || "Instagram"}</div>
                </div>
                <div className="rounded-lg border border-[#e1e4ea] p-3">
                  <div className="text-[12px] text-[#99a0ae] uppercase">Status</div>
                  <div className="mt-1 font-medium">{selected?.status || "Active"}</div>
                </div>
                <div className="rounded-lg border border-[#e1e4ea] p-3">
                  <div className="text-[12px] text-[#99a0ae] uppercase">Views</div>
                  <div className="mt-1 font-medium">{selected?.views || "—"}</div>
                </div>
                <div className="rounded-lg border border-[#e1e4ea] p-3">
                  <div className="text-[12px] text-[#99a0ae] uppercase">Likes</div>
                  <div className="mt-1 font-medium">{selected?.likes || "—"}</div>
                </div>
                <div className="rounded-lg border border-[#e1e4ea] p-3">
                  <div className="text-[12px] text-[#99a0ae] uppercase">Interactions</div>
                  <div className="mt-1 font-medium">{selected?.interactions || "—"}</div>
                </div>
              </div>
            </div>
          )}
